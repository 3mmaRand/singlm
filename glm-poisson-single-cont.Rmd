...
Towards the bottom of the output there is information about the model fit. The null deviance (what exists if we predict the number of cases from the mean, $\beta_{0}$) is `r null_dev` with `r df_null` degrees of freedom and the residual deviance (left over after our model is fitted) is `r resid_dev` with `r df_resid` $d.f.$. The model fits a 1 parameters and thus accounts for 1 $d.f.$ for a reduction in deviance by `r mod_dev`.
...
The number of cases reported by a clinic significantly decreases by a factor of `r b0_resp` for each kilometre from the nuclear plant (`r modelp`). See figure \@ref(fig:fig-cases-report).
