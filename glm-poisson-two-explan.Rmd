...
The intercept is the log of the expected number of prey items caught by 0 aged birds that spend the majority of their time in a group of many. The fact that the estimate for grouppair (`r b1`) is negative tells us that as such those in pairs catch fewer. Single birds also catch fewer. The positive coefficient for age indicates that more prey are caught with age.
...
Towards the bottom of the output there is information about the model fit. The null deviance (what exists if we predict the number of prey from the mean of birds in groups of many, $\beta_{0}$) is `r null_dev` with `r df_null` degrees of freedom and the residual deviance (left over after our model is fitted) is `r resid_dev` with `r df_resid` $d.f.$. The model fits a five parameters and thus accounts for 5 $d.f.$ for a reduction in deviance by `r mod_dev`.
...
All three terms in the model significantly reduce the deviance: group ($p=$`r group_p`), age ($p=$`r age_p`) and the interaction between them ($p=$`r interaction_p`). The effect of age is not the same for individuals in groups of different sizes.
