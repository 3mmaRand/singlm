...
<center> $\frac{P(ESR >20)}{P(ESR <20)}$ = `r b0_resp` $\times$ `r b1`$^{fibrinogen}$ </center>
...
The odds of erythrocyte sedimentation rate (ESR) greater than 20 mm/hr significantly increases with increasing fibrinogen levels by a factor of `r b1_resp` $\pm$ `r b1se` for each unit of fibrinogen (p `r modelp`). See Figure \@ref(fig:fig-plasma-report). As an example, the probability of having an ESR greater than 20 mm/hr at 2 units `r pred_2` is which rises to `r pred_6` at 6 units.

(ref:fig-plasma-report) Incidence erythrocyte sedimentation rate (ESR) greater than 20 mm/hr with fibrinogen levels. The line gives predictions for a GLM with binomial distributed errors, $\frac{P(ESR >20)}{P(ESR <20)}$ = `r b0_resp` $\times$ `r b1`$^{fibrinogen}$.
